...
- Most new AV-level and Sensor-level attacks can be integrated using the existing infrastructure without worrying about dataset collection. Details can be found in respective documentation (see above).
- If you implement a completely new attack, the key is to publish attack messages similar to how it is done currently in the lidar and bounding box modifier attacks so that the dataset generation script can read them the same way to label attacked timeframes. 
